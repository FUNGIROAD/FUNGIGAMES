angular.module( 'App.Views' ).controller( 'Discover.News.ListCtrl', function( $scope, $stateParams, App, Game_NewsArticle, gettextCatalog, payload )
{
	...
	if ( newsCtrl.section == 'hot' ) {
		App.title = gettextCatalog.getString( 'main.news.list.all_page_title' );
	}
	else if ( newsCtrl.section == 'followed' ) {
		App.title = gettextCatalog.getString( 'main.news.list.followed_page_title' );
	}
	else if ( newsCtrl.section == 'in-development' ) {
		App.title = gettextCatalog.getString( 'main.news.list.wip_page_title' );
	}
	else if ( newsCtrl.section == 'released' ) {
		App.title = gettextCatalog.getString( 'main.news.list.released_page_title' );
	}
} );
